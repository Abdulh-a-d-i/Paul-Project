...
BACKEND_API_URL = "https://backend.joinironfathers.com"
# os.getenv("BACKEND_API_URL", "http://3.135.250.76:8000")
...
async def _speak_status_update(ctx: RunContext, message: str, delay: float = 0.1):
    """Speak a brief status update before performing an action with minimal added latency."""
...
class SimpleOutboundCaller(Agent):
    def __init__(self, *, call_context: str, dial_info: dict[str, Any]):
        ...
        self.contact_first_name = dial_info.get("contact_first_name") or dial_info.get("first_name")
        self.contact_email = dial_info.get("contact_email") or dial_info.get("email")
        self.category = dial_info.get("category")
    ...
    def _build_fallback_prompt(self, call_context: str) -> str:
        ...
        """
        contact_name = self.contact_first_name or "there"
        return f"""You are {self.agent_name}, an AI assistant that makes phone calls to businesses on behalf of clients to book appointments and reservations.
                ...
                **Template:** "Hi {contact_name}! This is {self.agent_name} calling on behalf of {self.caller_name}. How are you doing today?"
                ...
                **USAGE PATTERN:**
                ...
                #### TOOL: submit_call_outcome
                **PURPOSE:** Save the call outcome status for reporting

                **WHEN TO USE:**
                - After the call is finished and before hanging up
                - Use one of: booked, call_again, do_not_call

                **USAGE PATTERN:**
                1. Decide the final status
                2. Say: "Let me note that down." (brief filler)
                3. Call: submit_call_outcome(status="booked"|"call_again"|"do_not_call")
                4. Proceed to end_call
    ...
    async def check_availability(
        ...
        start_time: str,
        end_time: str | None = None  # Make it optional with default None
    ):
        """
        Check if YOUR CLIENT is available at this time.
        Uses ONLY the pre-loaded cache - no API calls during conversation.
        
        Args:
            appointment_date: Date in YYYY-MM-DD format (e.g., "2025-12-23")
            start_time: Start time in HH:MM format (e.g., "18:00")
        ...
        try:
            # ✅ Auto-calculate end_time if not provided (default to 1 hour)
            ...
            # Convert proposed times to time objects for comparison
            try:
            ...
            # If cache is empty, client is free
            if len(self.appointments_cache) == 0:
                logger.info("Cache is empty - client is free")
                return {
                    "available": True,
            ...
            for apt in self.appointments_cache:
                ...
                try:
                    # Parse cached appointment times
                    ...
                    logger.warning(f"Could not parse cached time: {apt}. Skipping. Error: {e}")
                    continue
                ...
                if (proposed_start_t < apt_end_t and proposed_end_t > apt_start_t):
                    ...
                    logger.info(f"Conflict found: {apt}")
                    break
    ...
    async def book_appointment(
        ...
        try:
            ...
            # ✅ FIXED: Include phone_number for SMS
            payload = {
                ...
                "organizer_email": self.contact_email or self.caller_email,
                "phone_number": self.phone_number,  # ✅ Added for SMS
                "notes": notes or ""
            }
            ...
            if data.get("success"):
                ...
                google_status = " (saved to Google Calendar)" if data.get("google_calendar") else " (saved locally)"
                
                return {
                    "success": True,
                    "message": f"Successfully booked! {self.caller_name} is confirmed for {appointment_date} at {start_time}.{google_status}",
            ...
            return {"success": False, "error": f"Unable to book appointment: {str(e)}"}
    ...
    @function_tool()
    async def submit_call_outcome(self, ctx: RunContext, status: str):
        """Submit the call outcome status (booked, call_again, do_not_call) to the backend."""
        status_clean = (status or "").strip().lower()
        allowed = {"booked", "call_again", "do_not_call"}
        if status_clean not in allowed:
            return {"success": False, "message": f"Invalid status. Use one of {allowed}"}

        await _speak_status_update(ctx, "Got it, saving the call status...")

        payload = {"call_id": get_job_context().room.name if get_job_context() else None,
                   "status": status_clean}
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                resp = await client.post(f"{BACKEND_API_URL}/api/agent/update-call-outcome", json=payload)
                if resp.status_code == 200:
                    return resp.json()
                return {"success": False, "message": f"Backend responded with {resp.status_code}"}
        except Exception as e:
            logger.error(f"Failed to submit call outcome: {e}")
...
async def entrypoint(ctx: JobContext):
    ...
    try:
        ...
        target_name = agent.contact_first_name or agent.caller_name or "there"
        if language == "es":
            greeting = f"Hola {target_name}, soy {agent.agent_name} llamando de parte de {agent.caller_name}."
        else:
            greeting = f"Hi {target_name}, this is {agent.agent_name} calling on behalf of {agent.caller_name}."

        # Brief pause to let the callee say hello before the greeting lands
